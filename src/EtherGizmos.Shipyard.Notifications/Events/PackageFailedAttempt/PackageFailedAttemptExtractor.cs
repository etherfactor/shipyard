...
internal class PackageFailedAttemptExtractor : IDomainEventExtractor
{
    ...
    public PackageFailedAttemptExtractor(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (current == StatusTypeId.FailedAttempt
            && current != original)
        {
            ...
            var message = $"{package.Carrier.Name} package containing {package.Contents} had a failed delivery attempt with status: {lastUpdate?.Description}."
                .Replace("..", ".");

            var @event = new PackageFailedAttemptEvent()
            {
                ...
                Updates = [.. package.TrackingUpdates.Select(e => new PackageFailedAttemptEventUpdate()
                {
